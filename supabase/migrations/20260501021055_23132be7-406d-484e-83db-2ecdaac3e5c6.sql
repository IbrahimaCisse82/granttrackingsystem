CREATE INDEX IF NOT EXISTS idx_projects_org_archived ON public.projects(organization_id, archived);
CREATE INDEX IF NOT EXISTS idx_projects_pays ON public.projects(pays);
CREATE INDEX IF NOT EXISTS idx_projects_periodicite ON public.projects(periodicite);
CREATE INDEX IF NOT EXISTS idx_projects_created_at ON public.projects(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_projects_org_name ON public.projects(org);
CREATE INDEX IF NOT EXISTS idx_projects_debut ON public.projects(debut);
CREATE INDEX IF NOT EXISTS idx_projects_fin ON public.projects(fin);
...
BEGIN
  ...
  budget_lines_expanded AS (
    SELECT
      ...
      COALESCE((l->>'qty')::numeric, 0) * COALESCE((l->>'unitPrice')::numeric, 0) AS line_total
    FROM filtered f,
...
GRANT EXECUTE ON FUNCTION public.get_dashboard_metrics(uuid, text, text) TO authenticated;