...
const TYPE_META: Record<CalendarEventType, { label: string; color: string; icon: React.ReactNode }> = {
  report_deadline: { label: 'Rapport', color: '#DC2626', icon: <CalendarClock className="w-3.5 h-3.5" /> },
  payment: { label: 'Versement', color: '#059669', icon: <Wallet className="w-3.5 h-3.5" /> },
  project_end: { label: 'Fin projet', color: '#B45309', icon: <Flag className="w-3.5 h-3.5" /> },
  risk_review: { label: 'Risque', color: '#7C3AED', icon: <ShieldAlert className="w-3.5 h-3.5" /> },
};
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground mt-1">Deadlines rapports, versements, fins de projet et revues de risques.</p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as any)}
          ...
          <option value="report_deadline">Rapports</option>
          <option value="payment">Versements</option>
          <option value="project_end">Fins de projet</option>
          <option value="risk_review">Revues de risques</option>
        </select>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {selectedIso && (
            <div className="mt-4 border-t border-border pt-4">
              ...
              {selectedEvents.length === 0 ? (
                <p className="text-xs text-muted-foreground italic">Aucun événement ce jour.</p>
              ) : (
        ...
        <div className="rounded-[10px] border border-border bg-card p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">30 prochains jours</h3>
          {upcoming.length === 0 ? (
