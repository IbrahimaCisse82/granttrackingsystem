import { Project, Amendement, fmt } from '@/lib/mock-data';
import { useCallback, useState } from 'react';
import { Plus, FileText, Check, X, Send } from 'lucide-react';
import {
...
export default function ProjectAmendements({ project, onSave }: Props) {
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div>
                    ...
                    <textarea defaultValue={selected.justification} key={`${selectedIdx}-just-${selected.justification}`}
                      ...
                      rows={3}
                      className="w-full rounded-md border border-input bg-background px-3 py-2 text-xs outline-none focus:border-primary resize-none" />
                  </div>
                            ...
                            return (
                              <tr key={li} className="border-t border-rule hover:bg-muted/30">
                                <td className="px-3 py-2">
                                  <select value={line.code} onChange={e => updateLine(selectedIdx!, li, { code: e.target.value })}
                                    className="rounded border border-input bg-background px-2 py-1 text-xs font-mono outline-none">
                                    <option value="">—</option>
                                ...
                                <td className="px-3 py-2">
                                  <input type="number" defaultValue={line.delta} key={`${selectedIdx}-${li}-${line.delta}`}
                                    ...
                                    className="w-24 text-right font-mono text-xs rounded border border-input bg-background px-2 py-1 outline-none focus:border-primary" />
                                </td>
                                <td className="px-3 py-2">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {confirmAction?.action === 'approuve' && 'L\'amendement sera appliqué au budget. Cette action est définitive.'}
