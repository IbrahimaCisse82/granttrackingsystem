...
export default function ProjectBudget({ project, onSave, readOnly }: Props) {
  ...
  }, [project.budgetLines, onSave, readOnly]);
          ...
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="bg-ink-2">
                {['Code', 'Poste budgétaire', 'Unité', 'Qté', 'Montant unit. (FCFA)', 'Alloc. %', 'Total FCFA', 'Total EUR', ...(readOnly ? [] : [''])].map(h => (
            ...
            <tbody>
              ...
              {linesA.map((l) => {
                ...
                return <BudgetRow key={globalIdx} line={l} badge={project.color.badge} onUpdate={patch => updateLine(globalIdx, patch)} onRemove={() => removeLine(globalIdx)} readOnly={readOnly} />;
              })}
              ...
              {linesB.map((l) => {
                const globalIdx = project.budgetLines.indexOf(l);
                return <BudgetRow key={globalIdx} line={l} badge="b-amber" onUpdate={patch => updateLine(globalIdx, patch)} onRemove={() => removeLine(globalIdx)} readOnly={readOnly} />;
              })}
              <tr className="bg-ink text-sidebar-foreground font-mono font-bold text-xs">
...
function BudgetRow({ line, badge, onUpdate, onRemove, readOnly }: { line: BudgetLine; badge: string; onUpdate: (patch: Partial<BudgetLine>) => void; onRemove: () => void; readOnly?: boolean }) {
  ...
  return (
    <tr className="hover:bg-paper/50 transition-colors group">
      ...
      <td className="border-b border-rule-2 border-r px-3 py-2.5">
        <input type="text" defaultValue={line.desc} key={line.desc} disabled={readOnly} onChange={e => onUpdate({ desc: e.target.value })}
          className="w-full bg-transparent outline-none focus:bg-card focus:border-input rounded px-1 disabled:opacity-60 disabled:cursor-not-allowed" />
      </td>
      <td className="border-b border-rule-2 border-r px-3 py-2.5">
        <input type="text" defaultValue={line.unite} key={line.unite} disabled={readOnly} onChange={e => onUpdate({ unite: e.target.value })}
          className="w-20 bg-transparent text-muted-foreground outline-none focus:bg-card rounded px-1 disabled:opacity-60 disabled:cursor-not-allowed" />
      </td>
      <td className="border-b border-rule-2 border-r px-3 py-2.5">
        <input type="number" defaultValue={line.qty} key={line.qty} disabled={readOnly} onChange={e => onUpdate({ qty: Number(e.target.value) || 0 })}
          className="w-16 text-right font-mono bg-transparent outline-none focus:bg-card rounded px-1 disabled:opacity-60 disabled:cursor-not-allowed" />
      </td>
      <td className="border-b border-rule-2 border-r px-3 py-2.5">
        <input type="number" defaultValue={montantFcfa} key={montantFcfa} disabled={readOnly} onChange={e => onUpdate({ montant: (Number(e.target.value) || 0) / EUR_TO_FCFA })}
          className="w-28 text-right font-mono bg-transparent outline-none focus:bg-card rounded px-1 disabled:opacity-60 disabled:cursor-not-allowed" />
      </td>
      <td className="border-b border-rule-2 border-r px-3 py-2.5">
        <input type="number" defaultValue={line.allocation} key={line.allocation} disabled={readOnly} onChange={e => onUpdate({ allocation: Number(e.target.value) || 0 })}
          className="w-16 text-right font-mono bg-transparent outline-none focus:bg-card rounded px-1 disabled:opacity-60 disabled:cursor-not-allowed" />
      </td>
      ...
      {!readOnly && (
        <td className="border-b border-rule-2 px-3 py-2.5">
          <button onClick={onRemove} className="opacity-0 group-hover:opacity-100 focus-visible:opacity-100 text-dim hover:text-rose transition-all" title={i18n.t('ui.removeBudgetLine')} aria-label={`${i18n.t('ui.removeBudgetLine')} ${line.code ?? ''}`.trim()}>
