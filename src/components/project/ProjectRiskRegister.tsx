...
const STATUSES: { value: RiskStatus; label: string; cls: string }[] = [
  { value: 'open', label: 'Ouvert', cls: 'bg-rose/10 text-rose' },
  { value: 'mitigated', label: 'Mitigé', cls: 'bg-amber/10 text-amber' },
  { value: 'closed', label: 'Clos', cls: 'bg-emerald/10 text-emerald' },
];
...
export default function ProjectRiskRegister({ project, readOnly }: Props) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-base font-semibold flex items-center gap-2">
              <ShieldAlert className="w-4 h-4 text-rose" /> Registre des risques
            </h2>
            <p className="text-xs text-muted-foreground mt-1">
              Matrice 5×5 · score = probabilité × impact · conformité UE/AFD
            </p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-mono font-semibold">{avgOpen}</div>
            <div className="text-[10px] text-muted-foreground">score moyen (ouverts)</div>
          </div>
        </div>
      </div>
      ...
      {/* Heatmap 5x5 */}
      <div className="rounded-[10px] border border-rule bg-card p-4">
        <h3 className="text-[12px] font-semibold mb-3">Matrice probabilité / impact</h3>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {!readOnly && (
          <button onClick={() => setShowForm(v => !v)}
            ...
            <Plus className="w-3.5 h-3.5" /> Nouveau risque
          </button>
        )}
      </div>
      ...
      {showForm && !readOnly && (
        <div className="rounded-[10px] border border-primary/40 bg-card p-4 space-y-3">
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="rounded border border-input px-3 py-1.5 text-xs">Annuler</button>
            <button
      ...
      <div className="rounded-[10px] border border-rule bg-card overflow-x-auto">
        {shown.length === 0 ? (
          <p className="p-6 text-center text-sm text-muted-foreground italic">Aucun risque enregistré.</p>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-rule text-left text-[10px] uppercase tracking-wide text-muted-foreground">
                ...
                <th className="py-2 px-3 font-semibold">Revue</th>
            ...
            <tbody>
              {shown.map(r => (
                <tr key={r.id} className="border-b border-rule/50 align-top">
                  <td className="py-2 px-3">{CATEGORIES.find(c => c.value === r.category)?.label}</td>
                  ...
                  <td className="py-2 px-3 text-[11px]">{r.review_date ? new Date(r.review_date).toLocaleDateString('fr-FR') : '—'}</td>
                  <td className="py-2 px-3">
                    {!readOnly && (
                      <button onClick={() => confirm('Supprimer ce risque ?') && remove.mutate(r.id)}
