  ...
  return (
    <div className="mt-6 rounded-[10px] border border-rule bg-card">
      <div className="border-b border-rule px-4 py-3 flex items-center gap-2">
        ...
        <h3 className="text-[13px] font-semibold">{t('comments.title')} ({comments.length})</h3>
      </div>
      <div className="p-4 space-y-3">
        {isLoading ? (
          <div className="flex justify-center py-4"><Loader2 className="w-4 h-4 animate-spin text-primary" /></div>
        ) : comments.length === 0 ? (
          <p className="text-xs text-muted-foreground italic text-center py-3">{t('comments.empty')}</p>
        ) : (
          comments.map(c => (
            <div key={c.id} className={`rounded-lg p-3 text-xs ${c.userId === currentUserId ? 'bg-primary/5 border border-primary/10' : 'bg-muted/50 border border-rule'}`}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-muted-foreground">{new Date(c.createdAt).toLocaleString(i18n.language === 'en' ? 'en-GB' : 'fr-FR')}</span>
        ...
        <p className="text-[10px] text-muted-foreground">{t('comments.hint')}</p>
      </div>
    </div>
  );
}
